...
class CycleGAN_Generator(nn.Module):
    def __init__(self, input_nc: int=3, output_nc: int=3, ngf: int=64):
        ...
        model.append(conv(c_in=3, c_out=out_features, k_size=7, stride=1,pad=0, norm='in',activation='relu',bias=True))
        in_features = out_features
        for _ in range(n_downsampling):
            out_features *= 2
            model.append(conv(c_in=in_features, c_out=out_features, k_size=3, stride=2,pad=1, norm='in',activation='relu',bias=True))
            in_features = out_features
        # model.append(conv(c_in=64, c_out=128, k_size=3, stride=2,pad=1, norm='in',activation='relu',bias=True))
        # model.append(conv(c_in=128, c_out=256, k_size=3, stride=2,pad=1, norm='in',activation='relu',bias=True))
        # Residual Blocks
        ...
        for _ in range(2):
            out_features //= 2
            model.append(deconv(c_in=in_features, c_out=out_features, k_size=3, stride=2,pad=1, output_padding=1,norm='in',activation='relu',bias=True))  
            in_features = out_features
        # model.append(deconv(c_in=256, c_out=128, k_size=3, stride=2,pad=1, output_padding=1,norm='in',activation='relu',bias=True))
        # model.append(deconv(c_in=128, c_out=64, k_size=3, stride=2,pad=1, output_padding=1,norm='in',activation='relu',bias=True))
        
        model.append(nn.ReflectionPad2d(3))
        model.append(conv(c_in=64, c_out=3, k_size=7, stride=1,pad=0, norm=None,activation='tanh'))
...
class ResnetBlock(nn.Module):
    def __init__(self, dim: int):
        ...
        self.conv_block = nn.Sequential(
            ...
            conv(c_in=256, c_out=256, k_size=3, stride=1, pad=0, norm='in',activation='relu',bias=True),
            nn.ReflectionPad2d(1),
            conv(c_in=256, c_out=256, k_size=3, stride=1, pad=0, norm='in',activation=None,bias=True),
        )
        ### END YOUR CODE
...
class CycleGAN_Discriminator(nn.Module):
    def __init__(self, input_nc: int=3, ndf: int=64):
        ...
        model.append(conv(c_in=3, c_out=64, k_size=4, stride=2, pad=1, norm=None,activation='lrelu',bias=False))
        model.append(conv(c_in=64, c_out=128, k_size=4, stride=2, pad=1, norm='in',activation='lrelu',bias=True))
        model.append(conv(c_in=128, c_out=256, k_size=4, stride=2, pad=1, norm='in',activation='lrelu',bias=True))
        model.append(conv(c_in=256, c_out=512, k_size=4, stride=1, pad=1, norm='in',activation='lrelu',bias=True))
        model.append(conv(c_in=512, c_out=1, k_size=4, stride=1, pad=1, norm=None,activation=None))
        ### END YOUR CODE
...
class CycleGAN_Solver():
    def __init__(self, lr: float=0.0002, batch_size: int=1, num_workers: int=1):
        ...
        self.criterion_GAN: nn.Module = nn.MSELoss()
        self.criterion_cycle: nn.Module = nn.L1Loss()
        self.criterion_identity: nn.Module = nn.L1Loss()        
        ...
        ### YOUR CODE HERE (~ 3 lines)
        self.optimizerG: optim.Optimizer = optim.Adam(self.netG_A2B.parameters())
        self.optimizerD_A: optim.Optimizer = optim.Adam(self.netD_A.parameters())
        self.optimizerD_B: optim.Optimizer = optim.Adam(self.netD_B.parameters())
        ...
        ### YOUR CODE HERE (~ 1 line)
        self.trainloader, self.testloader = create_dataloader(dataset='summer2winter')
    ...
    def train(self, epochs: int=100):
        ...
        for epoch in range(epochs):
            for iter, (real_A, real_B) in enumerate(self.trainloader):
                ...
                loss_GAN_AB = self.criterion_GAN(self.netD_B(fake_B), real_label)
                fake_A = self.netG_B2A(real_B)
                loss_GAN_BA = self.criterion_GAN(self.netD_A(fake_A), real_label)
                ...
                loss_real = self.criterion_GAN(self.netD_A(real_A), real_label)
                loss_fake = self.criterion_GAN(self.netD_A(fake_A.detach()), fake_label)
                lossD_A = (loss_real + loss_fake) / 2
                ...
                ### YOUR CODE HERE (~ 4 lines)
                loss_real = self.criterion_GAN(self.netD_B(real_B), real_label)
                loss_fake = self.criterion_GAN(self.netD_B(fake_B.detach()), fake_label)
                lossD_B = (loss_real + loss_fake) / 2
