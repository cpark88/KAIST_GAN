...
class DCGAN_Generator(nn.Module):
    def __init__(self):
        ...
        model.append(deconv(c_in=256, c_out=256, k_size=4, stride=1,pad=0, norm='bn',activation='relu'))
        model.append(deconv(c_in=256, c_out=128, k_size=4, stride=2,pad=1, norm='bn',activation='relu'))
        model.append(deconv(c_in=128, c_out=64, k_size=4, stride=2,pad=1, norm='bn',activation='relu'))
        model.append(deconv(c_in=64, c_out=3, k_size=4, stride=2,pad=1, norm=None, activation='tanh'))
        ### END YOUR CODE
...
class DCGAN_Discriminator(nn.Module):
    def __init__(self, type: str='gan'):
        ...
        model.append(conv(c_in=256,c_out=1, k_size=4, stride=1, pad=0, norm=None,activation=None)) # activation?
...
class DCGAN_Solver():
    def __init__(self, type: str='gan', lr: float=0.0002, batch_size: int=64, num_workers: int=1, device=None):
        ...
        ### YOUR CODE HERE (~ 1 lines)
        self.trainloader, self.testloader = create_dataloader(dataset='cifar10')
    ...
    def train(self, epochs: int=100):
        ...
        for epoch in range(epochs):
            for iter, (real_img, _) in enumerate(self.trainloader):
                ...
                fake_img = self.netG(z) # Generate fake image batch with G


                if self.type == 'gan':
                    # output_real = nn.Sigmoid()(self.netD(real_img)).view(-1) # Forward pass real batch through D
                    # errD_real = self.criterion(output_real,real_label) # Calculate loss on all-real batch
                    # output_fake = nn.Sigmoid()(self.netD(fake_img.detach())).view(-1) # requires_grad=False in fake
                    # errD_fake = self.criterion(output_fake, fake_label) # Calculate D's loss on the all-fake batch
                    # lossD = errD_real + errD_fake
                    output_real = torch.sigmoid(self.netD(real_img)).view(-1) # Forward pass real batch through D
                    errD_real = self.criterion(output_real,real_label) # Calculate loss on all-real batch
                    output_fake = torch.sigmoid(self.netD(fake_img.detach())).view(-1) # requires_grad=False in fake
                    errD_fake = self.criterion(output_fake, fake_label) # Calculate D's loss on the all-fake batch
                    lossD = errD_real + errD_fake

                elif self.type == 'lsgan':
                    output_real = self.netD(real_img).view(-1) # Forward pass real batch through D
                    errD_real = self.criterion(output_real,real_label) # Calculate loss on all-real batch
                    output_fake = self.netD(fake_img.detach()).view(-1) # requires_grad=False in fake
                    errD_fake = self.criterion(output_fake, fake_label) # Calculate D's loss on the all-fake batch
                    lossD = -0.5 * (errD_real + errD_fake)

                elif self.type == 'wgan':
                    lossD = -torch.mean(nn.Sigmoid()(self.netD(real_img))) + torch.mean(nn.Sigmoid()(self.netD(fake_img)))
                
                elif self.type == 'wgan-gp':
                    alpha = torch.rand(batch_size, 1, 1, 1)
                    alpha = alpha.expand_as(real_img)
                    
                    interpolated = Variable(alpha * real_img + (1 - alpha) * fake_img, requires_grad=True)
                    prob_interpolated = nn.Sigmoid()(self.netD(interpolated))
                    lossD = -torch.mean(nn.Sigmoid()(self.netD(real_img))) + torch.mean(nn.Sigmoid()(self.netD(fake_img))) + GPLoss(self.device)(prob_interpolated,interpolated)
                ### END YOUR CODE
                ...
                self.netD.zero_grad()
                lossD.backward(retain_graph=True) # 원래 없었던 옵션 
                ...
                #fake_img = self.netG(z) # Generate fake image batch with G
                if self.type == 'gan':
                    output_fake = nn.Sigmoid()(self.netD(fake_img)).view(-1) 
                    lossG = self.criterion(output_fake, real_label) # Calculate G's loss on the all-fake batch
                elif self.type == 'lsgan':
                    
                    output_fake = self.netD(fake_img).view(-1) 
                    lossG = self.criterion(output_fake, real_label) # Calculate G's loss on the all-fake batch
...
class GPLoss(nn.Module):
    ...
    def forward(self, y: torch.Tensor, x: torch.Tensor):
        ...
        #fake = Variable(torch.Tensor(x.shape[0], 1).fill_(1.0), requires_grad=False)
        gradients = torch.autograd.grad(outputs=y, inputs=x,
                            grad_outputs=torch.ones(y.size()),#torch.ones(y.size()
        ...
        loss = LAMBDA * ((gradients_norm - 1) ** 2).mean()
...
def test_lossG_function(gan_type, lossG):
    ...
    # the first test
    if gan_type == 'gan':
        assert lossG.detach().allclose(torch.tensor(expected_lossG[0]), atol=10), \
            f"Generator ({gan_type}) Loss of the model does not match expected result." # |lossG-1.5416| <= 0.01 #1e-2
    # the second test
    elif gan_type == 'lsgan':
        assert lossG.detach().allclose(torch.tensor(expected_lossG[1]), atol=10), \
            f"Generator ({gan_type}) Loss of the model does not match expected result."
    # the third test
    elif gan_type == 'wgan':
        assert lossG.detach().allclose(torch.tensor(expected_lossG[2]), atol=10), \
            f"Generator ({gan_type}) Loss of the model does not match expected result."
    # the fourth test
    elif gan_type == 'wgan-gp':
        assert lossG.detach().allclose(torch.tensor(expected_lossG[3]), atol=10), \
            f"Generator ({gan_type}) Loss of the model does not match expected result."
...
def test_lossD_function(gan_type, lossD):
    ...
    if gan_type == 'gan':
        assert lossD.detach().allclose(torch.tensor(expected_lossD[0]), atol=1e-2), \
            f"Discriminator ({gan_type}) Loss of the model does not match expected result." #1e-2
    # the second test
    elif gan_type == 'lsgan':
        assert lossD.detach().allclose(torch.tensor(expected_lossD[1]), atol=10), \
    ...
    # the fourth test
        ...
        assert lossD.detach().allclose(torch.tensor(expected_lossD[3]), atol=10), \
            f"Discriminator ({gan_type}) Loss of the model does not match expected result."
